...
import * as exerciseSelectors from "src/app/components/exercise-editor/exercise-editor.selectors";
import { Preset } from "src/app/models/preset.model";
import { Exercise } from "src/app/models/exercise.model";
import { ExerciseEditorComponent } from "src/app/components/exercise-editor/exercise-editor.component";
import * as countdonwSelectors from "../../components/countdown/countdown.selectors";
...
import { sortBySeqNo } from "../../components/exercise-editor/exercise-editor.reducer";
import {
  DialogOptions,
  DialogTitleTypes,
...
export class PresetEditorComponent implements OnInit, OnDestroy {
  ...
  @ViewChild("repetitionsInput", { static: false })
  repetitionsInput: ElementRef<HTMLInputElement>;
  exercises: Exercise[];
  ...
  ngOnInit() {
    this.ar.queryParamMap.subscribe((paramMap: ParamMap) => {
      const presetId = paramMap.get("pId");

      this.presetSubscription = this.store
        .pipe(select(fromSelectors.selectPreset(presetId)))
        .subscribe((preset) => {
          if (preset) {
            this.preset = preset;
            ...
            this.countdownsSubscription = this.store
              .pipe(
                select(
                  countdonwSelectors.allCountdownsOfExercises(
                    this.preset.exercisesIds,
                  ),
                ),
              )
  ...
  private openDialog(exercise: Exercise, opts: { title; isNew }): void {
    const dref = this.dialog.open(ExerciseEditorComponent, {
  ...
  sharePreset(): void {
    let data: string = this.share.construct(
      this.preset,
      this.exercises,
      this.countdowns,
    );
  ...
  openLinkDialog(data) {
    this.dialog.open(TextCopyComponent, {
      ...
      width: "80%",
      data: {
        link: `${new URL(document.location.href).origin}/export?preset=${data}`,
      },
    });
  }
